...
const SignDocumentPage = () => {
  ...
  useEffect(() => {
    const fetchDocument = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE}/documents/${documentId}/sign/${token}`);
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      // Update document state with multi-party information from backend
      if (data.document) {
        setDocument(prev => ({
          ...prev,
          requiresSecondParty: data.document.requiresSecondParty,
          secondPartyLabel: data.document.secondPartyLabel,
          firstPartyLabel: data.document.firstPartyLabel
        }));
      }
    } catch (err) {
      setError(err.message);
  ...
  const handleSendSecondParty = async () => {
    if (!secondPartyEmail.trim()) return;

    setSendingSecondParty(true);
    try {
      const response = await fetch(`${API_BASE}/documents/${documentId}/send-second-party`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ secondPartyEmail: secondPartyEmail.trim() }),
      });
      ...
      setSecondPartySent(true);
    } catch (err) {
      setError(err.message);
    } finally {
      setSendingSecondParty(false);
    }
  };
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <Card className="w-full max-w-md p-6 text-center">
          ...
          <p className="text-muted-foreground mb-4">
            Your electronic signature has been recorded and a confirmation email has been sent.
          </p>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600">Electronic Document Signing Portal</p>
          </div>
          ...
          <Card className="mb-6 p-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-gray-700">
                <strong>Template:</strong> {document?.templateTitle}
              </p>
              <p className="text-gray-700">
